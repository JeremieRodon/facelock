...
The D-Bus system bus policy (`/usr/share/dbus-1/system.d/org.facelock.Daemon.conf`) governs who may own the bus name and which methods each caller may send. Three grants (ADR 010): root may send anything on the interface; every local user may send exactly one method, `org.facelock.Daemon.Authenticate`, which is what lets screen lockers and the polkit agent unlock with no group membership and no re-login; the `facelock` group may receive the daemon's signals and nothing more. (`/etc/dbus-1/system.d/` is the admin-override location for local customization.)
